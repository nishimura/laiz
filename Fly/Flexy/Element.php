...
// | Original Author: Alan Knowles <alan@akbkhome.com>                    |
...
class Fly_Flexy_Element {
     ...
     * @access   public
     */
    function Fly_Flexy_Element($tag='', $attributes=null)
    {
    ...
    function attributesToHTML()
    {
        $strAttr = '';
        $xhtmlclose = '';
        $charset = empty($GLOBALS['Fly_Flexy']['options']['charset']) ? 'ISO-8859-1' : $GLOBALS['Fly_Flexy']['options']['charset'];
    ...
    function setValue($value) {
        ...
        switch ($tag) {
            ...
            case 'textbox':
            case 'checkbox':
                require_once 'Fly/Flexy/Element/Xul.php';
                Fly_Flexy_Element_Xul::setValue($this,$value);
                return ;
    ...
    function setOptions($array,$noValue=false) 
    {
        ...
        if (strlen($tag) && ($tag != 'select')) {
                require_once 'Fly/Flexy/Element/Xul.php';
                return Fly_Flexy_Element_Xul::setOptions($this,$array,$noValue);
        }
        
        foreach($array as $k=>$v) {
            if (is_array($v)) {     // optgroup
                $child = new Fly_Flexy_Element($namespace . 'optgroup',array('label'=>$k));
                foreach($v as $kk=>$vv) {
                    $atts=array();
                    if (($kk !== $vv) && !$noValue) {
                        $atts = array('value'=>$kk);
                    ...
                    $add = new Fly_Flexy_Element($namespace . 'option',$atts);
