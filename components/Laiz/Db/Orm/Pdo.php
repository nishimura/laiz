 ...
 * @author    Satoshi Nishimura <nishim314@gmail.com>
 */
class Laiz_Db_Orm_Pdo implements Laiz_Db_Orm{
     ...
     * @access public
     */
    public function __construct(Laiz_Db_Driver $db, $tableName = null){
        $this->dao = $db;
        if ($tableName)
    ...
    private function _createVo($tableName){
        ...
        $className = 'Laiz_Db_Vo_'. $voClassSuffix;

        if (!class_exists($className, false)){
            // Create VO instance
            $fileName = $this->getComponentDir() . 'Laiz/Db/Vo/'. $voClassSuffix . '.php';
