...
abstract class Laiz_Error
{
    ...
    public function getBackTrace(){
        ...
        for ($i = 0; $i < count($b); $i++){
            ...
            // エラー処理クラスのトレースは表示しない
            if (__FILE__ == $b[$i]['file'])
                continue;
            if (dirname(__FILE__).'/Error' == dirname($b[$i]['file']))
                continue;
    ...
    static public function getInstance($args = array()){
        if (self::$instance === null){
            self::$instance = new LaizErrorUtils();
            self::$instance->init(E_USER_ERROR|E_USER_WARNING|E_USER_NOTICE);
        }
