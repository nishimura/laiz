...
class Fly_Flexy_Factory {
    ...
    */
    ...
    function fromArray($ar,$ret=array())
    {
        ...
        foreach($ar as $k=>$v) {
            if (is_array($v)) {
                $ret = Fly_Flexy_Factory::fromArrayPrefixed($k,$v,$ret);
            }
    ...
    */
    ...
    function fromArrayPrefixed($prefix, $ar,$ret=array())
    {
        ...
        foreach($ar as $k=>$v) {
            if (is_array($v)) {
                $ret = Fly_Flexy_Factory::fromArrayPrefixed($prefix.'['.$k.']',$v,$ret);
                if (!isset($ret[$prefix.'['.$k.'][]'])) {
                    $ret[$prefix.'['.$k.'][]'] = new Fly_Flexy_Element();
                }
    ...
    function &setErrors(&$ret,$set,$format='<span class="error">%s</span>')
    {
        ...
        if (!is_array($set)) {
            return Fly_Flexy::raiseError(
                'invalid arguments "$set" (should be an array) sent to Fly_Flexy_Factory::setErrors',
                0, FLY_FLEXY_ERROR_DIE);
        }
        foreach($set as $k=>$v) {
            if (!$v) {
                continue;
